...
public class BasicQuery<T, ID extends Serializable> implements JpaRepository<T, ID> {
    ...
    @Override
    public <S extends T> List<S> findAll(Pageable pageable, String sortName, String sortBy) {
        Connection connection = ((ConnectionPoolImpl)(connectionPool)).takeOut();
        StringBuilder sql = new StringBuilder(Query.SELECT)
                .append(AnnotationUtil.getTableName(tClass));
                if (sortName != null && sortBy != null && StringUtils.isNotBlank(sortName) && StringUtils.isNotBlank(sortBy)){
    ...
    @Override
    public <S extends T> List<S> findAllById(int idUser) {
        Connection connection = ((ConnectionPoolImpl)(connectionPool)).takeOut();
        StringBuilder sql = new StringBuilder(Query.SELECT).append(AnnotationUtil.getTableName(tClass)).append(" where id_user = "+idUser+"");
        PreparedStatement ps = null;
        try {
            ps = connection.prepareStatement(sql.toString());
            ResultSet resultSet = ps.executeQuery();
